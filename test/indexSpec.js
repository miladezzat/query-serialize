...
const compareObject = (obj1 = {}, obj2 = {}) =>
  expect(_.isEqual(obj1, obj2));



describe('Get /', () => {
  it('return success with no quey sent', function () {
    request(app)
      .get('/')
      .then(response => {
        expect(200);
  ...
  it('return success with quey sent', function () {
    request(app)
      ...
      .then(response => {
        ...
        expect(response.body).to.be.an('object').and.have
          .keys(['$and']);
        const { body } = response;
        const existObj = { "$and": { text: 'hello', limit: 5, page: 1, status: true } }
